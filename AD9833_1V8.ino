   ...
   do {
    ...
    if (fstart < fstop) {
      ...
      delayMicroseconds(stp_time);
        if (csteps == ln) {
          digitalWrite(5, HIGH); delayMicroseconds(2); digitalWrite(5, LOW); delayMicroseconds(2);
          csteps = 0;
        }
      csteps++; 
      }
...
void handlerLN(char* param) 
{
  ln = strtod(param, nullptr);
  Serial.print("\nSet a marker after ");Serial.print(ln); Serial.print(" steps");
}
...
const MD_cmdProcessor::cmdItem_t PROGMEM cmdTable[] =
{
  ...
  { "ln", handlerLN,    "n",  "After how many steps do you want a marker: ", 4},
...
void setup()
{
  ...
  pinMode(5, OUTPUT); digitalWrite(5, LOW); // LSB of marker
