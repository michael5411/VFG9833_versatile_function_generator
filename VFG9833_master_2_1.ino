...
{ 
  ...
  String mode_str = param.substring(colon4 + 1); // find mode 
...
{ 
  ...
  String fstop_str = param.substring(colon1 + 1, colon2); // find 2nd value
  String step_str = param.substring(colon2 + 1, colon3); // find step value
  String mode_str = param.substring(colon3 + 1); // find mode 

  long fstart = fstart_str.toInt(); // first value to Integer
  long fstop = fstop_str.toInt(); // 2nd value to Integer
  long step = step_str.toInt(); // 3rd value to Integer 
  int mode = mode_str.toInt(); // 4th value to Integer

  Serial.println("Resonce finding is done with these parameters:");
  Serial.print("\nStart frequency [Hz] <fstart>: ");
  Serial.println(fstart);
  Serial.print("Stop frequency   [Hz] <fstop>: ");
  Serial.println(fstop);
  ...
  if (fstart < fstop) {
    ...
    for (f;f <= ffstop; f = f + stp) {
      ...
      if (mode == 1){
        ...
        Serial.print("   Output voltage: ");Serial.print(outv); Serial.println(" [Vpp]");
        }
      else{
         Serial.print(outv);Serial.print(";"); Serial.println(f);
        }
      }
    }
    else {
...
const MD_cmdProcessor::cmdItem_t PROGMEM cmdTable[] =
{
  ...
  { "mc", handlerMC,    "c", "Set AD9833 to masterclock frequency [Hz]", 3},
  { "sw", handlerSW,    "s", "Sweep Parameter fstart:fstop:step:step_time:mode", 4},
  { "re", handlerRE,    "r", "Find Resonance between fstart:fstop:step:mode 1/0", 4},
